...
class SMScodeState extends State<SMScode> {
          ...
          children: <Widget>[
            ...
            Padding(
                padding: EdgeInsets.only(
                    top: 20.0, bottom: 20.0, left: 50, right: 50),
                child:
                    pincode.genTextFields(context, _controller, 6, 50.0, 35.0)),
            StreamBuilder<bool>(
  ...
  verifyPhone(BuildContext context) async {
    ...
    PhoneAuthCredential credential = PhoneAuthProvider.credential(
        verificationId: verificationId, smsCode: pincode.getValues());

    // Sign the user in (or link) with the credential
    auth.signInWithCredential(credential).then((user) {
      print('signed in with phone number successful: user -> $user');
      Navigator.push(context, MaterialPageRoute(builder: (_) {
        return Book(user.user.phoneNumber);
      }));
    }).catchError((onError) {
